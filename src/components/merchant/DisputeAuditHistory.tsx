...
const ACTION_LABELS: Record<string, string> = {
  "dispute.evidence_uploaded": "Evidence uploaded",
  "dispute.evidence_deleted": "Evidence removed",
  "dispute.response_submitted": "Response submitted",
  "dispute.submission_failed": "Submission failed",
};

export default async function DisputeAuditHistory({ finixDisputeId, churchId }: { finixDisputeId: string; churchId: string }) {
  const entries = await prisma.dashboardAuditLog.findMany({
    where: { churchId, entityType: "dispute", entityId: finixDisputeId },
