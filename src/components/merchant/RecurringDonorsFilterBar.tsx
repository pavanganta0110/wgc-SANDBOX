...
export default function RecurringDonorsFilterBar({ exportHref }: { exportHref: string }) {
  ...
  const minMonthly = searchParams.get("minMonthly") || "";
  ...
  const activeFilterCount =
    [status, frequency, minMonthly, maxMonthly].filter(Boolean).length + (hasFailedPayment ? 1 : 0) + (hasPastDue ? 1 : 0) + (requiresAttention ? 1 : 0);
  ...
  return (
    <div className="flex items-center gap-3 mb-4 flex-wrap">
      <input
        type="text"
        placeholder="Search name, email, phone, subscription ID"
        value={searchInput}
      ...
      <div className="relative">
        ...
        >
          {status ? STATUSES.find((s) => s.value === status)?.label : "Recurring Status"}
          ...
          <>
            ...
            <div className="absolute left-0 mt-2 z-50 bg-white rounded-2xl border border-slate-200 shadow-xl p-4 w-80 space-y-3">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1">Min Monthly Value ($)</label>
                  <input type="text" value={minMonthly} onChange={(e) => setParam("minMonthly", e.target.value)} className="w-full px-3 py-1.5 rounded-lg border border-slate-200 text-sm outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1">Max Monthly Value ($)</label>
                  <input type="text" value={maxMonthly} onChange={(e) => setParam("maxMonthly", e.target.value)} className="w-full px-3 py-1.5 rounded-lg border border-slate-200 text-sm outline-none" />
                </div>
              </div>
