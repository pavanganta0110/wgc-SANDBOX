...
import { loadDonorInstrumentIds, loadDonorGivingLinksTab } from "@/lib/donors/donorTabs";
...
export async function GET(req: Request, { params }: { params: Promise<{ donorId: string }> }) {
  ...
  const givingLinks = await loadDonorGivingLinksTab(instrumentIds, session.churchId);
  return NextResponse.json({ givingLinks });
}
