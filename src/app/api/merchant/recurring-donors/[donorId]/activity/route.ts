...
import { loadDonorInstrumentIds, loadDonorActivityTab } from "@/lib/donors/donorTabs";
...
export async function GET(req: Request, { params }: { params: Promise<{ donorId: string }> }) {
  ...
  const events = await loadDonorActivityTab(donor, instrumentIds, session.churchId);
