...
export async function POST(req: Request, { params }: { params: Promise<{ clientId: string }> }) {
  ...
  const existing = await prisma.client.findFirst({ where: { id: clientId, churchId: auth.churchId } });
  if (!existing) {
  ...
  if (!body.acknowledgeDuplicates) {
    const duplicates = await findPossibleDuplicateClients(auth.churchId, { normalizedEmail, normalizedPhone, organizationName }, clientId);
