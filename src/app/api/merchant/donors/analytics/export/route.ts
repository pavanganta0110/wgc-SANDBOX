...
export async function GET(req: Request) {
  ...
  lines.push(
    `Summary,Total Donated,${csvEscape(formatCents(summary.totalDonatedCents))}`,
  );
  lines.push(
    `Summary,WGC Processed,${csvEscape(formatCents(summary.wgcProcessedCents))}`,
  );
  lines.push(
    `Summary,External Donated,${csvEscape(formatCents(summary.externalDonatedCents))}`,
  );
  lines.push(
    `Summary,Average Donation,${csvEscape(formatCents(summary.averageDonationCents))}`,
  );
  lines.push(
    `Summary,Donors With Failed Payments,${summary.donorsWithFailedPayments}`,
  );
  lines.push(
    `Summary,Donors Requiring Attention,${summary.donorsRequiringAttention}`,
  );
  lines.push(`New vs Returning,New Donors,${extended.newVsReturning.newCount}`);
  lines.push(
    `New vs Returning,Returning Donors,${extended.newVsReturning.returningCount}`,
  );
  lines.push(
    `One-Time vs Recurring,One-Time Donations,${extended.oneTimeVsRecurring.oneTimeCount}`,
  );
  lines.push(
    `One-Time vs Recurring,Recurring Donations,${extended.oneTimeVsRecurring.recurringCount}`,
  );
  lines.push(
    `Retention,Returning Donor Rate,${extended.retention.returningDonorRate ?? "N/A"}`,
  );
  lines.push(`Retention,Retained Donors,${extended.retention.retainedDonors}`);
  lines.push(
    `Concentration,Top 10% Donor Share,${extended.concentration.top10SharePct}%`,
  );
  ...
  for (const t of trend) {
    lines.push(
      [
        t.period,
        formatCents(t.grossDonatedCents),
        formatCents(t.netDonatedCents),
        t.donationCount,
        t.uniqueDonorCount,
      ]
  ...
  for (const g of growth) {
    lines.push(
      [g.period, g.newDonors, g.returningDonors, g.totalActiveDonors]
        .map((v) => csvEscape(String(v)))
        .join(","),
    );
  }
